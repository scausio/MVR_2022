...
#CUT ON NEA AND NWP
argo = argo.where((argo['longitude'].astype(float) > -45) &
                  (argo['longitude'].astype(float) < 155) &
                  (argo['latitude'].astype(float) >5) &
                  (argo['latitude'].astype(float) < 70)).dropna(dim='obs')

#DECIMATE
# idx=[int(i)for i in np.linspace(0,argo.sizes['obs'],1000)][:-1]
# argo=argo.isel(obs=idx)


argo = argo.where((argo['pressure_qc'].astype(float) == 1) &
                  (argo['time_qc'].astype(float) == 1) &
                  (argo['temperature_qc'].astype(float) == 1) &
                  (argo['salinity_qc'].astype(float) == 1)).dropna(dim='obs')
