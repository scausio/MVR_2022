...
from tasks.utils import getMonthsFirst_LastDays,getMonthsFirst_NextMonthFirstDays,getMonthsFirst_NextMonthFirstDays_plusminus2, getJID
import subprocess
...
def main():
    ...
    ##end_date=f'{to_year_month}{monthrange(int(to_year_month[:4]), int(to_year_month[4:])+1)[1]:02d}'
    #a=monthrange(int(to_year_month[:4])+1, 1)
    print (f'Running MVR from {start_date} to {end_date}')
    #months = getMonthsFirst_LastDays(start_date, end_date)
