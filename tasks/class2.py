...
class Class2():
    ...
    def main(self):
        ...
        for forecast in self.config.forecasts:
            ds_mod=xr.open_mfdataset(os.path.join(self.config_base.output,'ts_cl2',self.config.forecasts[forecast].filename).format(date=f"{self.ref_date}*", p_date=f"{self.ref_date}*"))
            for var in conf.variables:
                for plat_name in self.conf[variables].platforms:
                    ...
                    outds.to_netcdf(os.path.join(self.config_base.output,self.ref_date,f"class2_{plat_name}_{stat_interm.model.values[0]}_{outds.attrs['start_date']}_{outds.attrs['end_date']}_{var}.nc"),
